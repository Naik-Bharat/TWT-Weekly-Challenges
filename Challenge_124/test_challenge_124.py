...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_steps = max(len(x.split()) for x in test_out)
    print(
        f" - Max steps needed: {yellow}{max_steps}{reset} for x = {yellow}"
        f"{next(int(x.split()[0]) for x in test_out if len(x.split()) == max_steps):_}{reset}"
    )
    print(
        f" - Average steps needed: {yellow}"
