...
class Config:
    ...
    TEST_CASE_FILE = "test_cases.json"      #  1k test cases
    # TEST_CASE_FILE = "test_cases_b.json"  # 10k test cases
...
def print_extra_stats(
    ...
    print(f" - max number of numbers: {yellow}" f"{max(len(x[0].split()) // 2 + 1 for x in test_inp):_}{reset}")
    print(
        f" - Average number of numbers: {yellow}"
        f"{sum(len(x[0].split()) // 2 + 1 for x in test_inp) // num_cases:_}{reset}"
    )
