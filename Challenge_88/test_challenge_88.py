...
def print_extra_stats(
    ...
    print(
        f" - Min number length: {yellow}"
        f"{min(len(x[0]) for x in test_inp):_}{reset}"
    )
    print(
        f" - Max number length: {yellow}"
        f"{max(len(x[0]) for x in test_inp):_}{reset}"
    )
    print(
        f" - Average number length: {yellow}"
