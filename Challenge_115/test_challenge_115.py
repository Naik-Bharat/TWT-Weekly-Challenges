...
class Config:
    ...
    TEST_CASE_FILE = "test_cases.json.gz"  # 1000 test cases max n 10_000
    # TEST_CASE_FILE = "test_cases_b.json.gz"  # 300 test cases max n 100_000
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    print(f" - Max number of Queries: {yellow}" f"{max(int(x[0]) for x in test_inp):_}{reset}")
    print(
        f" - Average number of Queries: {yellow}"
        f"{sum(int(x[0]) for x in test_inp) // num_cases:_}{reset}"
    )
    print(
        f" - Max prints: {yellow}" f"{max(len(x.split()) for x in test_out):_}{reset}"
    )
    print(
        f" - Average prints: {yellow}"
        f"{sum(len(x.split()) for x in test_out) // num_cases:_}{reset}"
    )
