...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    print(
        f" - Max word length: {yellow}"
        f"{max(max(map(len, x.split())) for x in test_out):_}{reset}"
    )
    print(
        f" - Average word length: {yellow}"
        f"{sum(sum(map(len, x.split())) // len(x.split()) for x in test_out) // num_cases:_}{reset}"
    )
