...
class Config:
    ...
    TEST_CASE_FILE = "test_cases.json"          # 1000 test cases with max length of the heights 1015
    # TEST_CASE_FILE = "test_cases_b.json.gz"   # 500 test cases with max length of the heights 32550
...
def print_extra_stats(
    ...
    print(
        f" - Heights max length: {yellow}"
        f"{max(len(x[1].split()) for x in test_inp):_}{reset}"
    )
    print(
        f" - Heights min length: {yellow}"
        f"{min(len(x[1].split()) for x in test_inp):_}{reset}"
    )
    print(
        f" - Heights average length: {yellow}"
        f"{sum(len(x[1].split()) for x in test_inp) // num_cases:_}{reset}"
    )
