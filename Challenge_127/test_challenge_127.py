...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "Solution.rs"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    print(f" - Max N: {yellow}" f"{max(int(x[0].split()[0]) for x in test_inp):_}{reset}")
    print(f" - Max D: {yellow}" f"{max(int(x[0].split()[1]) for x in test_inp):_}{reset}")
    print(
        f" - Average N: {yellow}"
        f"{sum(int(x[0].split()[0]) // 2 for x in test_inp) // num_cases:_}{reset}"
    )
    print(
        f" - Average D: {yellow}"
        f"{sum(int(x[0].split()[1]) // 2 for x in test_inp) // num_cases:_}{reset}"
    )
