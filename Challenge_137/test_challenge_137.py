...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "Solution.go"
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    ...
    max_s = max(int(x) for x in test_out)
    avg_s = sum(int(x) for x in test_out) // num_cases

    print(f" - Max string length: {yellow}{max_n:_}{reset}")
    print(f" - Average string length: {yellow}{avg_n:_}{reset}")
    print(f" - Max number of swaps: {yellow}{max_s:_}{reset}")
    print(f" - Average number of swaps: {yellow}{avg_s:_}{reset}")
