...
class Config:
    ...
    TEST_CASE_FILE = "test_cases.json"         # 1001 cases max B 4_981
    # TEST_CASE_FILE = "test_cases_b.json.gz"  #  501 cases max B 49_546
...
def print_extra_stats(
    ...
    print(
        f" - Max B: {yellow}"
        f"{max(int(x[0].split()[2]) for x in test_inp):_}{reset}"
    )
    print(
        f" - Average N: {yellow}"
    ...
    print(
        f" - Average B: {yellow}"
        f"{sum(int(x[0].split()[2]) for x in test_inp) // num_cases:_}{reset}"
    )
