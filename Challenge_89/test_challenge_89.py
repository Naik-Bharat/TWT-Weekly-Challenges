...
class Config:
    ...
    TEST_CASE_FILE = "test_cases.json.gz"  # 1000 cases with max array size 100x100
    # TEST_CASE_FILE = "test_cases_b.json.gz"  # 100 cases with max array size 500x500
...
def print_extra_stats(
    ...
    print(
        f" - Min array elements: {yellow}"
        f"{min(int(x[0].split()[0]) * int(x[0].split()[1]) for x in test_inp):_}{reset}"
    )
    print(
        f" - Max array elements: {yellow}"
        f"{max(int(x[0].split()[0]) * int(x[0].split()[1]) for x in test_inp):_}{reset}"
    )
    print(
        f" - Average array elements: {yellow}"
        f"{sum(int(x[0].split()[0]) * int(x[0].split()[1]) for x in test_inp) // num_cases:_}{reset}"
    )
